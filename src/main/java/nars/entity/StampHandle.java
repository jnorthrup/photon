 ...
 */
public class StampHandle implements Cloneable, Stamp {
     ...
     */
    public StampHandle(StampHandle old) {
     ...
     */
    public StampHandle(StampHandle old, long time) {
        baseLength = old.length();
        evidentialBase = old.getBase();
        creationTime = time;
    }
     ...
     */
    public StampHandle(StampHandle first, StampHandle second, long time) {
        int i1, i2, j;
        i1 = i2 = j = 0;
     ...
     */
    public static StampHandle make(StampHandle first, StampHandle second, long time) {
        ...
        if (first.length() > second.length()) {
            return new StampHandle(first, second, time);
        } else {
            return new StampHandle(second, first, time);
        }
    }
    ...
    public boolean equals(Object that) {
        if (!(that instanceof StampHandle)) {
            return false;
        }
        TreeSet<Long> set1 = toSet();
        TreeSet<Long> set2 = ((StampHandle) that).toSet();
