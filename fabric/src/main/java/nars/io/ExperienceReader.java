...
public class ExperienceReader implements InputChannel {
	...
	public void openLoadFile() {
		FileDialog dialog = new FileDialog((FileDialog) null,
				"Load experience", FileDialog.LOAD);
	...
	public void openLoadFile(String filePath) {
		...
		ReasonerBatch.addInputChannel(reasoner, this);
	}
	...
	public void closeLoadFile() {
		...
		ReasonerBatch.removeInputChannel(reasoner, this);
	}
		...
		while (timer == 0) {
			...
			if (line.length() > 0) {
				try {
					...
					ReasonerBatch.walk(reasoner, timer);
				} catch (NumberFormatException e) {
					ReasonerBatch.textInputLine(reasoner, line);
				}
			}
		}
		return true;
