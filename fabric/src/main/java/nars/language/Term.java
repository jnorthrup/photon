 ...
 */
public class Term implements Cloneable, Comparable<Term>, TermStruct {
    ...
    public static boolean hasOrIsTerm$par(Term root, Term... leaves) {
        return Arrays.stream(leaves).anyMatch(term -> Objects.equals(root, term)) || root instanceof CompoundTerm && ((CompoundTerm) root).getComponents().stream().anyMatch(t -> hasOrIsTerm$par(t, leaves));
    }
