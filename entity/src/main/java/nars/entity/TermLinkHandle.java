 ...
 */
public class TermLinkHandle extends ItemAtomic implements TermLink {
     ...
     */
    public TermLinkHandle(Term t, short p, int... indices) {
        target = t;
        type = p;
        assert (type % 2 == 0); // template types all point to compound, though the target is component
        if (type == TermLinkHandle.COMPOUND_CONDITION) {  // the first index is 0 by default
     ...
     */
    public TermLinkHandle(String s, BudgetValueAtomic v) {
        super(s, v);
    }
     ...
     */
    public TermLinkHandle(Term t, TermLink template, BudgetValueAtomic v) {
        super(t.getName(), v);
        target = t;
